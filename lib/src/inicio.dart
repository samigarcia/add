...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _loadNotes() async {
    final dbHelper = DatabaseHelper(); // Instancia de DatabaseHelper para interactuar con la base de datos
    final notes = await dbHelper.getAllNotes(); // Obtener las notas desde la base de datos

    setState(() {
      _notes = notes; // Actualiza la lista de notas en el estado con las notas cargadas desde la base de datos
    });
  }
              ...
              /*Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      SizedBox(height: 24),
                      for (int index = 0; index < getNotesForSelectedCategory().length; index++)
                            Padding(
                              padding: EdgeInsets.only(left: 20,top: 10, right: 20),
                              child: Row(
                                children: [
                                      ...
                                      child: Column(
                                        children: [
                                          ...
                                          Container(
                                            ...
                                            height: 100,
                                            decoration: BoxDecoration(
                                                color: catColor.getColorByIndex(_notes[index].categoryId),
                                                borderRadius: BorderRadius.circular(8)
                                            ),
                                            child: Padding(
                                              padding: EdgeInsets.only(left: 20, right: 20),
